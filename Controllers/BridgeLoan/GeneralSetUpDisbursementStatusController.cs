...
namespace Adroit_v8.Controllers.BridgeLoan
{
    ...
    [ApiController]
    [Authorize]
    public class GeneralSetUpDisbursementStatusController :AuthController
    {
        private readonly IMongoRepository<GeneralSetUpDisbursementStatus> _repo;
        string errMsg = "Unable to process request, kindly try again";
      //  string _CreatedBy = "0";
        public GeneralSetUpDisbursementStatusController(IMongoRepository<GeneralSetUpDisbursementStatus> repo, IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
        {
            _repo = repo;
        }
        ...
        public async Task<IActionResult> Add([FromBody] DocSetupFm obj)
        {
            ...
            try
            {
                GeneralSetUpDisbursementStatus la = new()
                {
                    DocName = obj.Name,
                    Status = obj.Status,
        ...
        [NonAction]
        private List<GeneralSetUpDisbursementStatus> Get()
        {
