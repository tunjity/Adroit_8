...
namespace Adroit_v8.Controllers.BridgeLoan
{
    ...
    [ApiController]
    [Authorize]
    public class GeneralSetUpDocumentationStageController :AuthController
    {
        private readonly IMongoRepository<GeneralSetUpDocumentationStage> _repo;
        string errMsg = "Unable to process request, kindly try again";
      //  string _CreatedBy = "0";
        public GeneralSetUpDocumentationStageController(IMongoRepository<GeneralSetUpDocumentationStage> repo, IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
        {
            _repo = repo;
        }
        ...
        public async Task<IActionResult> Add([FromBody] DocSetupFm obj)
        {
            ...
            try
            {
                GeneralSetUpDocumentationStage la = new()
                {
                    DocName = obj.Name,
                    Status = obj.Status,
        ...
        [NonAction]
        private List<GeneralSetUpDocumentationStage> Get()
        {
