...
namespace Adroit_v8.Controllers.BridgeLoan
{
    ...
    [ApiController]
    [Authorize]
    public class DocumentSetUpController :AuthController
    {
        private readonly IMongoRepository<DocSetup> _repo;
        string errMsg = "Unable to process request, kindly try again";
        //string _CreatedBy = "0";
        public DocumentSetUpController(IMongoRepository<DocSetup> repo, IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
        {
            _repo = repo;
        }
        ...
        public async Task<IActionResult> GetAll([FromQuery] PaginationWithOutFilterModel obj)
        {
            ...
            try
            {
                var query = Get(); var fneRes = query.Skip((obj.PageNumber - 1) * obj.PasgeSize)
                    .Take(obj.PasgeSize);
                if (query.Any())
                    eget = true;
        ...
        [SwaggerResponse(StatusCodes.Status500InternalServerError, Type = typeof(ReturnObject))]
        [Route("getallvalid")]
        public async Task<IActionResult> GetAllValid([FromQuery] PaginationWithOutFilterModel obj)
        {
            var r = new ReturnObject(); bool eget = false;
            try
            {
                var query = Get().Where(o => o.IsDeleted == 0).ToList(); var fneRes = query.Skip((obj.PageNumber - 1) * obj.PasgeSize)
                    .Take(obj.PasgeSize);
                if (query.Any())
                    eget = true;
                r.status = eget ? true : false;
