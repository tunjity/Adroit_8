...
namespace Adroit_v8.Controllers.BridgeLoan
{
    ...
    [ApiController]
    [Authorize]
    public class DocumentationStatusController : AuthController
    {
        private readonly IMongoRepository<DocumentationStatus> _repo;
        string errMsg = "Unable to process request, kindly try again";
        // string _CreatedBy = "0";
        public DocumentationStatusController(IMongoRepository<DocumentationStatus> repo, IHttpContextAccessor httpContextAccessor) : base(httpContextAccessor)
        {
            _repo = repo;
        }
        ...
        public async Task<IActionResult> GetAllValid([FromQuery] PaginationWithOutFilterModel obj)
        {
            ...
            try
            {
                ...
                var query = Get().Where(o => o.IsDeleted == 0);
                eget = true;
                r.status = eget ? true : false;
                r.message = eget ? "Record Fetched Successfully" : "No Record Found";
                r.data = query.ToList();
        ...
        [NonAction]
        private List<DocumentationStatus> Get()
        {
